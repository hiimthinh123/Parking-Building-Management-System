...
function ManagerSidebar() {
    ...
    const username = user ? user.fullName : 'MANAGER';
                ...
                <div className="w-100">
                    ...
                    <div className="text-center mb-4 px-2 py-3 rounded-3" style={{ background: 'linear-gradient(180deg, #111827 0%, rgba(17,24,39,0) 100%)' }}>
                        <div style={{ fontSize: '11px', letterSpacing: '0.8px', color: '#64748B' }} className="fw-bold text-uppercase">
                            <i className="fa-regular fa-id-badge me-1.5 text-cyan-accent"></i> Xin Chào, Quản Lý
                        </div>
                    ...
                    <nav className="nav flex-column mt-2 w-100" style={{ whiteSpace: 'nowrap' }}>
                        ...
                        <NavLink to="/manager/dashboard" className={navLinkClass}>
                            {({ isActive }) => (<><i className={iconClass(isActive, 'fa-solid fa-chart-pie')}></i> Trang Tổng Quan</>)}
                        </NavLink>
                        ...
                        <NavLink to="/manager/floors" className={navLinkClass}>
                            {({ isActive }) => (<><i className={iconClass(isActive, 'fa-solid fa-layer-group')}></i> Quản Lý Tầng / Slot</>)}
                        </NavLink>

                        <NavLink to="/manager/tariff" className={navLinkClass}>
                            {({ isActive }) => (<><i className={iconClass(isActive, 'fa-solid fa-tags')}></i> Cấu Hình Bảng Giá</>)}
                        </NavLink>

                        <NavLink to="/manager/reports" className={navLinkClass}>
                            {({ isActive }) => (<><i className={iconClass(isActive, 'fa-solid fa-file-invoice-dollar')}></i> Báo Cáo Doanh Thu</>)}
                        </NavLink>
                    </nav>
                </div>
